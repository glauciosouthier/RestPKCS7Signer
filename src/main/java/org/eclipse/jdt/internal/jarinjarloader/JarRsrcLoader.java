...
public class JarRsrcLoader {
	...
	public static void main(String[] args) throws ClassNotFoundException,
		...
		for (int i = 0; i < mi.rsrcClassPath.length; i++) {
			...
			if (rsrcPath.endsWith(JIJConstants.PATH_SEPARATOR))
				rsrcUrls[i] = new URL(
						JIJConstants.INTERNAL_URL_PROTOCOL_WITH_COLON
								+ rsrcPath);
			else
				rsrcUrls[i] = new URL(
		...
		Method main = c.getMethod(JIJConstants.MAIN_METHOD_NAME,
	...
	private static ManifestInfo getManifestInfo() throws IOException {
			...
			try {
				...
				if (is != null) {
					...
					result.rsrcMainClass = mainAttribs
							.getValue(JIJConstants.REDIRECTED_MAIN_CLASS_MANIFEST_NAME);
					String rsrcCP = mainAttribs
							.getValue(JIJConstants.REDIRECTED_CLASS_PATH_MANIFEST_NAME);
					if (rsrcCP == null)
		...
		System.err
				.println("Missing attributes for JarRsrcLoader in Manifest (" + JIJConstants.REDIRECTED_MAIN_CLASS_MANIFEST_NAME + ", " + JIJConstants.REDIRECTED_CLASS_PATH_MANIFEST_NAME + ")"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		return null;
	}
